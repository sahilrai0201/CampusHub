...
const ManageNotices = () => {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const res = await api.put(`/admin/notices/${editingId}`, {
        title: editTitle,
  ...
  return (
    <DashboardLayout>
      <div className="space-y-8 font-sans">
        <div>
          <h1 className="text-2xl font-bold font-display text-gray-100">Campus Notice Board</h1>
          <p className="text-gray-400 text-sm mt-1">Publish bulletins and announcements for students and faculty members</p>
        </div>
                          ...
                          <div className="flex justify-between items-start gap-4">
                            <div>
                              ...
                              <span className="text-[10px] text-gray-500 mt-1 block">
                                Posted by: <span className="text-blue-400 font-semibold">{not.createdBy?.name || 'Admin'}</span> ({not.createdBy?.role})
                              </span>
                            </div>
                            
                            {/* Actions (Allow delete if Admin or if creator is Admin) */}
